  ...
  function bindMenuButtons() {
    document.querySelectorAll('[data-action]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        else if (a === 'local') showScreen('local');
      });
    });
  ...
  function attachNetworkListeners() {
    Network.setListeners({
      ...
      onAction: ({ peerId, action }) => {
        if (state.mode !== 'host') return;
        // Find which seat this peer belongs to
        ...
        // After applying, if next player is bot, schedule bot turn
